...
public class GameBoard {
   ...
   * Constructor
   */
  public GameBoard(int player, int player_reversi, int opponent_reversi) {
    ...
    for(int i = 0; i < move_count[disks_played]; ++i){
      try{
        if( gb.getOccupation( ArraytoCoordinate( move_list[disks_played][i] ) ) == OPPONENT_REVERSI){
          if(OPPONENT == RED)
            make_move(RED, move_list[disks_played][i]);
          else
            make_move(GREEN, move_list[disks_played][i]);
        }
      }
   ...
   */
  private reversi.Coordinates ArraytoCoordinate(int square){
